...
public class DroolsCompilerAntTask extends MatchingTask {
    ...
    private void serializeObject(Object object) throws
            IOException {
    ...
    private void compileAndAddFile(PackageBuilder builder, String fileName)
        ...
        try {
            ...
            if (fileName.endsWith(DroolsCompilerAntTask.RULEFLOWMODELFILEEXTENSION)
                    || fileName.endsWith(DroolsCompilerAntTask.RULEFLOWFILEEXTENSION)) {
                builder.addRuleFlow(fileReader);
            } else if (fileName.endsWith(DroolsCompilerAntTask.XMLFILEEXTENSION)) {
                builder.addPackageFromXml(fileReader);
            } else if (fileName.endsWith(DroolsCompilerAntTask.BRLFILEEXTENSION)) {
                builder.addPackageFromBrl(ResourceFactory.newReaderResource(fileReader));
            } else if (fileName
...
//                System.out.println(drl);
                ...
                builder.addPackageFromDrl(fileReader);
            }
        } finally {
            if (fileReader != null) {
                fileReader.close();
